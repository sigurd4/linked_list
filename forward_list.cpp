...
namespace sss
{
    ...
    template<typename T>
    template<typename F> requires sss::comparison<F, T>
    constexpr forward_list<T>& forward_list<T>::sort_par(F&& predicate, size_t threads) noexcept
    {
        // Do nothing if length is 0 or 1
        if(!this->first.has_value() || !this->first.value().next_link().has_value())
        {
    ...
    template<typename T>
    template<typename F> requires sss::comparison<F, T>
    constexpr forward_list<T>& forward_list<T>::sort_par(const F& predicate, size_t threads) noexcept
    {
        // Do nothing if length is 0 or 1
        if(!this->first.has_value() || !this->first.value().next_link().has_value())
        {
